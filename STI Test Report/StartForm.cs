...
namespace STI_Test_Report
{
    public partial class StartForm : Form
    {
        ...
        void CreateReport(string pdfFilePath, string dbFilePath, bool optionNotRun, bool optionNotApplicable, bool optionLastTestRun, bool optionNoTestResults, bool optionNoTestSteps, List<int> testSuiteIds)
        {
            ...
            //    bool optionNotRun = Request.Form.Keys.Contains("optionNotRun") && Request.Form["optionNotRun"] == "1";
            //    bool optionNotApplicable = Request.Form.Keys.Contains("optionNotApplicable") && Request.Form["optionNotApplicable"] == "1";
            //    bool optionLastTestRun = Request.Form.Keys.Contains("optionLastTestRun") && Request.Form["optionLastTestRun"] == "1";
            //    bool optionNoTestResults = Request.Form.Keys.Contains("optionNoTestResults") && Request.Form["optionNoTestResults"] == "1";
            //    bool optionNoTestSteps = Request.Form.Keys.Contains("optionNoTestSteps") && Request.Form["optionNoTestSteps"] == "1";
            //    string testSuiteFilter = Request.Form["textTestSuiteFilter"];

            //    List<int> testCaseIds = null;
            //    foreach (var key in Request.Form.Keys)
            //    {
            //        if (!key.StartsWith("testCase_"))
            //            continue;
            //        if (int.TryParse(key.Substring(9), out int testCaseId))
            //        {
            //            if (testCaseIds == null)
            ...
            //    }
            //    List<int> testSuiteIds = null;
            //    if (!string.IsNullOrEmpty(testSuiteFilter))
            //    {
            //        testSuiteIds = new List<int>();
            //        var idFields = testSuiteFilter.Split(',');
            //        foreach (var idField in idFields)
            //            if (int.TryParse(idField, out int id))
            //                testSuiteIds.Add(id);
            //    }

            //    var sqlLiteBL = new SQLiteConnector.BL();
            //    sqlLiteBL.OpenDatabase(filePath);
            //    var testPlan = sqlLiteBL.TestPlanRead();
            //    var pdfReport = new OutPdf.PdfSTITestReport();
            ...
            //        return NoContent();
            //    stream.Position = 0;
            //    return File(stream.ToArray(), "application/pdf", $"TestReport_{model.TestPlanId}.pdf");
        }
